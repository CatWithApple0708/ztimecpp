...
class CoreSystemState : public Object {
 ...
 public:
  ...
  /**
   * @brief Get the Thread Info
   * object,如果是自己创建的线程应该在线程创建的时刻,调用createThreadInfo
   *
   * @param id
  ...
  shared_ptr<ExceptionHandler> setDefaultExceptionHandler(
 ...
 private:
  string threadIdToStr(pthread_t id) { return to_string(id); };
};
}  // namespace core
}  // namespace zwsd
