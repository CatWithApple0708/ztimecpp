...
void testSendPacket(int sendPacketNum, int sendTimes) {
  uint8_t sendBuf[1024 * 3] = {0};
  size_t sendLength = 0;
  ...
  EXPECT_EQ(receive_packet, sendPacketNum);
}

void testSendPacketWithErrorPacket(int sendPacketNum, int sendTimes) {
  ...
  if (remain_size != 0) {
    UPProcessPushRawDataRet_t ret = UP_process_push_raw_data(
        &uart_protocol_handler, sendBuf + each_packet_size * sendPacketNum,
        remain_size);
    EXPECT_TRUE(ret.success);
    call_up_process_raw_data(
        &uart_protocol_handler,
