...
Thread::Thread(string name, function<void()> run, shared_ptr<ThreadStateListener> listener)
{
	...
	workThread.reset(new thread([this]() {
		...
		catch (const std::exception &exception)
		{
			...
			auto listener = this->listener.lock();
			if (listener)
				listener->onCatchException(this, exception);
			else
				callDefaultExceptionHandler(exception);
		}
		catch (...)
		{
			/**
			...
			auto listener = this->listener.lock();
			if (listener)
				listener->onCatchException(this, unkownTypeException);
			else
				callDefaultExceptionHandler(unkownTypeException);
		}
	}));
...
void Thread::sleep()
{
	...
	if (threadInfo)
		...
		throw BaseException(BaseException::format1024("Thread %s not call SystemState::Instance().createThreadInfo(name)", to_string(pthread_self()).c_str()));
}
...
const weak_ptr<ThreadStateListener> &Thread::getListener() const
{
	return listener;
}
void Thread::callDefaultExceptionHandler(const std::exception &exception) {
	auto handler = SystemState::Instance().getThreadExceptionHandler();
	if (handler){
