...
namespace argh
{
   ...
   public:
      ...
      parser(const char* const argv[], int mode = SINGLE_DASH_IS_MULTIFLAG)
      {  parse(argv, mode); }

      parser(int argc, const char* const argv[], int mode = SINGLE_DASH_IS_MULTIFLAG)
      {  parse(argc, argv, mode); }

      void add_param(std::string const& name);
      void add_params(std::initializer_list<char const* const> init_list);

      void parse(const char* const argv[], int mode = SINGLE_DASH_IS_MULTIFLAG);
      void parse(int argc, const char* const argv[], int mode = SINGLE_DASH_IS_MULTIFLAG);
     ...
     for (auto i = 0u; i < args_.size(); ++i) {
       ...
       if (!(mode & NO_SPLIT_ON_EQUALSIGN)) {
         ...
         if (equalPos != std::string::npos) {
           params_.insert(
               {name.substr(0, equalPos), name.substr(equalPos + 1)});
           continue;
         }
       }
