...
bool FileUtil::isDirectory(const string &path) {
  ...
  if (hFile == INVALID_HANDLE_VALUE) return false;
  FindClose(hFile);
  if (result.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) return true;
  return false;
}

bool FileUtil::makeDirIfNoExist(const string &path) {
  BOOL fret = CreateDirectory(path.c_str(), NULL);
  if (fret == FALSE && ERROR_ALREADY_EXISTS != GetLastError()) return false;
  return true;
}
...
bool FileUtil::delFile(const string &filePath) {
  ...
  if (hFinder == INVALID_HANDLE_VALUE) return false;
  ...
  if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
    ...
    while (1) {
      ...
      if (fda.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        if (strcmp(fda.cFileName, ".") != 0 &&
            strcmp(fda.cFileName, "..") != 0) {
          // recurse the delete.
          delFile(fname.c_str());
        }
      } else {
...
bool FileUtil::writeToFile(const string &fileName,
                           const std::vector<char> &buf) {
  std::ofstream outfile1(fileName, std::ios::trunc);
  if (outfile1.is_open()) {
    outfile1.write((char *)buf.data(), buf.size());
...
bool FileUtil::writeToFile(const string &fileName, const string &buf) {
  vector<char> vc(buf.data(), buf.data() + buf.size());
...
FileUtil::DataFileHelper::DataFileHelper(const string &fileDir)
    : mFileDir(fileDir) {
...
bool FileUtil::DataFileHelper::openReadFile(const string &filePath,
                                            bool inCurrentDir) {
  string fullPath;
...
void FileUtil::DataFileHelper::createWriteFile(const string &filename,
                                               const string &suffix,
...
void FileUtil::DataFileHelper::createWriteFile(const string &suffix,
                                               bool append) {
  string filename = getCurrentTime();
  createWriteFile(filename, suffix, append, true);
}

bool FileUtil::DataFileHelper::write(const char *data, int dataLen, int offset,
                                     int writeLen) {
