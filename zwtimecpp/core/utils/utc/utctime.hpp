 ...
 * gitSite: https://github.com/paulgriffiths/utctime.git
 */
...
class UTCTimeException {
 ...
 public:
   ...
   */
  explicit UTCTimeException(const std::string msg = "No error message")
      : m_error_message(msg) {}
...
class bad_time_init : public UTCTimeException {
 public:
  //! Constructor
  explicit bad_time_init() : UTCTimeException("Couldn't initialize time"){};
};
...
class bad_time : public UTCTimeException {
 public:
  //! Constructor
  explicit bad_time() : UTCTimeException("Couldn't get time"){};
};
...
class invalid_date : public UTCTimeException {
 public:
  //! Constructor
  explicit invalid_date(const std::string& msg) : UTCTimeException(msg){};
};
...
bool check_utc_timestamp(const time_t check_time, int& secs_diff,
                         ...
                         const int hour, const int minute, const int second);
time_t get_utc_timestamp(const int year, const int month, const int day,
                         const int hour, const int minute, const int second);
int get_utc_timestamp_sec_diff(const time_t check_time, const int year,
...
class UTCTime {
 public:
  ...
  explicit UTCTime(const int year, const int month, const int day,
                   const int hour, const int minute, const int second);
...
static inline OStream& operator<<(OStream& os, const UTCTime& utcTime) {
  return os << utcTime.time_string();
}
}
}  //  namespace utctime
#endif  // PG_UTC_TIME_H
