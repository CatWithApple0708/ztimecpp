...
class UartProtocolV1Packet {
 ...
 public:
  ...
  static shared_ptr<UartProtocolV1Packet> createShakeHandPacket(
    ...
    return UP_construct_packet_wappter(packet, serial_num, kShakeHandPacket);
  }
  /**
   ...
   * @return shared_ptr<UartProtocolV1Packet>
   */
  static shared_ptr<UartProtocolV1Packet> createPingPacket(
      uint16_t serial_num) {
    shared_ptr<UartProtocolV1Packet> packet(new UartProtocolV1Packet());
    packet->config.ping_packet = UPPingPacket_construct_config();
    return UP_construct_packet_wappter(packet, serial_num, kPingPacket);
  }
  ...
  static shared_ptr<UartProtocolV1Packet> createReceipt(
      uint16_t serial_num, UP_error_code_t error_code, T... args) {
    shared_ptr<UartProtocolV1Packet> packet(new UartProtocolV1Packet());
    ...
    return UP_construct_packet_wappter(packet, serial_num, kReceiptPacket);
  }
  /**
  ...
  static shared_ptr<UartProtocolV1Packet> createHardwareOperatePacket(
      uint16_t serial_num, UP_module_type_code_t moduleType, uint8_t moduleNum,
      UP_operate_code_t operate_code, uint16_t dataPoint, T... args) {
    shared_ptr<UartProtocolV1Packet> packet(new UartProtocolV1Packet());
    UPParametersContainer_initialize(&packet->paramterContainer);
    UPParametersContainer_push_value_wappter(&packet->paramterContainer,
                                             args...);
    packet->config.hardware_operate_packet =
        UPHardwareOperatePacket_construct_config(moduleType, moduleNum,
                                                 ...
                                                 &packet->paramterContainer);
    return UP_construct_packet_wappter(packet, serial_num, kHardwareOperatePacket);
  }

  template <class... T>
  static shared_ptr<UartProtocolV1Packet> createSystemEventPacket(
      uint16_t serial_num, UP_GenernalSystemEvent_t event, T... args) {
    shared_ptr<UartProtocolV1Packet> packet(new UartProtocolV1Packet());
    UPParametersContainer_initialize(&packet->paramterContainer);
    UPParametersContainer_push_value_wappter(&packet->paramterContainer,
                                             args...);
    ...
    return UP_construct_packet_wappter(packet, serial_num, kSystemEventReportPacket);
  }
 ...
 private:
  static shared_ptr<UartProtocolV1Packet> UP_construct_packet_wappter(
      ...
      PacketType_t type) {
    packet->basicPacket = UPBasicPacketConfig_construct(serial_num, type);
